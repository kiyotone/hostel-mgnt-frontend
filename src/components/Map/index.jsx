...
const Map = () => {
    ...
    useEffect(()=>{
        ...
        navigator.geolocation.getCurrentPosition((location)=>{
            ...
            setCurrentLocation({lat:Number(location.coords.latitude) , lng:Number(location.coords.longitude),})
        })
        console.log(hostels)
        getHostels();
  ...
  return (
    ...
    <div className=''>
        ...
        <GoogleMap key={12} zoom={20} center={currentLocation} options={options} mapContainerClassName='w-full h-[40rem] rounded-md' >
        <MarkerF key={12} position={currentLocation} />
            {hostels && hostels.map((marker)=>{
                return  ( 
                    <div key={marker._id}>
                    
                    <MarkerF key={marker._id} position={{lat:Number(marker.latitude) , lng:Number(marker.longitude)}} clickable onClick={()=>setSelectedMarker(marker)} options={markerOptions}/>
